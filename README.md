...
- [4, 5, 6].forEach(obj.logValues): obj.logValues 함수 자체를 forEach의 콜백으로 전달함. forEach는 이 콜백을 호출할 때 별도의 thisArg를 지정하지 않았으므로, logValues 함수 내의 this는 전역 객체를 참조하게 됨
...
- 또한, obj1.func를 obj3을 this로 하여 호출(obj1.func.call(obj3))하고 그 반환값을 callback3에 저장함
...
- return된 함수의 클로저의 메모리 관리 방법을 보여줌
...
- 함수 스코프와 클로저를 활용하여 객체의 private 멤버와 public 메서드를 구현하는 방법을 보여줌

- 외부에서는 public 메서드를 통해서만 private 멤버에 접근하고 조작할 수 있도록 함
...
```
//실행 결과
2km 이동 (총 2km, 남은 연료: 16.6)
undefined
undefined
undefined
1000
6km 이동 (총 8km, 남은 연료: 15.4)
100
5km 이동 (총 13km, 남은 연료: 14.4)
1000
...
- arguments 객체와 apply 메서드를 사용하여 부분 적용 함수를 직접 구현하는 방법
- 원본 함수의 this 컨텍스트를 유지하면서, 미리 전달된 인자들과 새로 전달된 인자들을 조합하여 함수를 실행
...
var curry3 = function (func) {
  ...
  var getMaxWith10 = curry5(Math.max)(10);
